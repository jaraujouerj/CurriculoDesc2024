...
\ementa	            {Computação de alto desempenho: CPUs multinucleadas (\textit{multi-core}); programação de propósito geral em unidades de processamento gráfico (GPGPU); computadores paralelos; multiprocessadores; multicomputadores; aglomerados computacionais (\textit{clusters}) e grades computacionais (\textit{grids}); computação em nuvem. Conceitos de Sistemas Distribuídos. Programação Paralela: desenvolvimento de programas paralelos com \textit{threads} e memória compartilhada; desenvolvimento de programas distribuídos com troca de mensagens. Ambientes bibliotecas para programação paralela e distribuída.
}
