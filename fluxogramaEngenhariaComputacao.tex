...
\newcounter{totalCredDiscExtensao} 
\setcounter{totalCredDiscExtensao}{0} 
...
\newcommand{\disciplina}[7] {
  ...
  \ifthenelse{\equal{#1}{extensao}}
    {\addtocounter{totalCredDiscExtensao}{#6}} % Se for extensao, conta também como crédito de extensão
...
\begin{tikzpicture}[remember picture, overlay]
  \node[anchor=south west, text width=6cm, align=justify]
...
\setcounter{horasDiscExtensao}{\inteval{15 * \the\value{totalCredDiscExtensao}}}

% conta horas totais do curso sem contar extensão
\newcounter{horasSemExtensao}
\setcounter{horasSemExtensao}{\inteval{15 * (\the\value{cred} - \the\value{totalCredDiscExtensao})}}
...
\newcounter{hEletivas}
\setcounter{hEletivas}{\inteval{60 * \the\value{tEletivas}}}

% total  horas disciplinas obrigatórias, exceto eletivas
\newcounter{hObrigatorias}
\setcounter{hObrigatorias}{\inteval{\the\value{horasTotaisDisciplinas} - \the\value{hEletivas}}}
...
\begin{tikzpicture}[remember picture, overlay]
  \node[anchor=south west, text width=10cm, align=justify]
  at ([xshift=16cm, yshift=1cm]current page.south west)
  {O currículo do curso de Engenharia de Computação
    ...
    \begin{enumerate}[label=\alph*)] % isso gera a), b), c), etc.
      \item \the\value{hObrigatorias} horas, equivalentes a {\inteval{\the\value{hObrigatorias} / 15}} créditos em disciplinas obrigatórias, sendo \the\value{horasDiscExtensao} horas, \the\value{totalCredDiscExtensao} créditos, de Disciplinas obrigatórias com carga horária de Extensão;
      \item um mínimo de \the\value{hEletivas} horas, 12 créditos em disciplinas eletivas restritas;
      \item um mínimo de \the\value{horasACE} horas em Atividades Curriculares de Extensão;
    \end{enumerate}
  };
\end{tikzpicture}

%O currículo do curso de Engenharia de Elétrica com ênfase em Sistemas Elétricos e de Automação Industrial
%compreende um total de 4100 horas, 261 créditos, sendo:
%a) 3795 horas, equivalentes a 253 créditos de Disciplinas Obrigatórias, contendo:
%%- 165 horas, 11 créditos, de Disciplina de Estágio Obrigatório; e
%- 225 horas, 15 créditos, de Disciplinas obrigatórias com carga horária de Extensão;
%b) um mínimo de 60 horas, 04 créditos, de Disciplinas Eletivas Restritas;
%c) um mínimo de 60 horas, 04 créditos, de Disciplinas Eletivas Definidas;e
%d) um mínimo de 185 horas de Atividades Curriculares de Extensão.
...
% \the\value{cred} créditos em disciplinas obrigatórias\\ sendo \the\value{totalCredDiscExtensao} créditos em disciplinas de extensão\\e 12 créditos em disciplinas eletivas restritas
...
\immediate\write\exportfile{% Total de horas do curso: totalHorasCurso}
\immediate\write\exportfile{% Total de créditos do curso: totalCredCurso}
\immediate\write\exportfile{% Total de horas de disciplinas (regulares+eletivas+extensao): horasTotaisDisciplinas}
\immediate\write\exportfile{% Total de horas de disciplinas obrigatórias (regulares+extensao): hObrigatorias}
\immediate\write\exportfile{% Total de horas de disciplinas eletivas: hEletivas}
\immediate\write\exportfile{% Total de horas de extensão: horasExtensao}
\immediate\write\exportfile{% Total de horas de disciplinas de extensão: horasDiscExtensao}
\immediate\write\exportfile{% Total de créditos disciplinas de extensao: totalCredDiscExtensao}
\immediate\write\exportfile{% Total de horas de atividades de extensão (sem disciplinas): totalHorasACE}
\immediate\write\exportfile{% Total de horas sem extensao: totalHorasSemExtensao}
\immediate\write\exportfile{% Total de disciplinas: totalDisciplinas}
\immediate\write\exportfile{% Total de disciplinas eletivas: totalEletivas}
\immediate\write\exportfile{% Total de disciplinas obrigatorias: tDiscObrigatorias}
\endgroup
...
\immediate\write\exportfile{%
  ...
  \noexpand\newcommand{\string\hObrigatorias}{\thehObrigatorias\noexpand\xspace}^^J%
  ...
  \noexpand\newcommand{\string\totalCredDiscExtensao}{\thetotalCredDiscExtensao\noexpand\xspace}^^J%
